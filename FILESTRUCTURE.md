...
* C++ source files:
  * `include/bit_chess_cpp/bit_chess_cpp.hpp` is the main
    C++ header that declares the interface of your library.
  * `app/bit_chess_cpp_app.cpp` is an executable that uses the library.
    This can e.g. be used to provide a command line interface for your project.
  * `tests/bit_chess_cpp_t.cpp` contains the unit tests for the library.
